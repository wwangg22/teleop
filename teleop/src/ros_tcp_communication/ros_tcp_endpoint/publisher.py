...
class RosPublisher(RosSender):
    ...
    def send(self, data):
        ...
        """
        # Let rclpy do the CDR decoding. The hand-rolled convert_data() parser
        # ignored the CDR encapsulation header and field alignment, which shifted
        # every float and produced garbage values (denormals / 1e152 magnitudes).
        try:
            message = deserialize_message(data, type(self.msg))
            self.pub.publish(message)
        except Exception as e:
            self.get_logger().error(
                "Failed to deserialize message for topic {}: {}".format(self.topic, e)
            )
