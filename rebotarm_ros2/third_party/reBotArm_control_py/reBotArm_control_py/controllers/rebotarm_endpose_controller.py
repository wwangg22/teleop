...
class RebotArmEndPose:
    ...
    def __init__(
        ...
        # Gravity-feedforward cache for the MIT loop (see _loop_cb).
        self._grav_q_ref: Optional[np.ndarray] = None
        self._grav_tau: np.ndarray = np.zeros(self._n)
        ...
        if self._arm_group:
            if self._arm_control_mode == "mit":
                ...
                if self._use_gravity_ff:
                    # Gravity is evaluated at the TARGET posture, not at a
                    # fresh position read. get_positions() here would issue one
                    # synchronous 0x7019 param round-trip PER ROBSTRIDE JOINT
                    # (six blocking CAN transactions inside this loop body),
                    # which caps the "500 Hz" loop at an irregular 50-160 Hz.
                    # _q_target is seeded from the measured pose before this
                    # loop starts and callers keep it within their command-lag
                    # bound of the arm, so the posture error is small and the
                    # feedforward stays consistent with the setpoint.
                    q_ref = self._q_target
                    if (self._grav_q_ref is None
                            or float(np.max(np.abs(q_ref - self._grav_q_ref))) > 0.02):
                        q_pad = pad_q_for_model(self._model, q_ref, self._n)
                        tau = compute_generalized_gravity(
                            self._model, q_pad, self._data)[: self._n].copy()
                        tau[1] *= 1.55  # joint2 额外补偿
                        tau[2] *= 1.55  # joint3 额外补偿
                        self._grav_q_ref = q_ref.copy()
                        self._grav_tau = tau
                    tau_ff = self._grav_tau
