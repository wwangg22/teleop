...
class JointGroup:
    ...
    def get_temperatures(
        ...
        """Per-joint (t_mos, t_rotor) in degrees C.

        MotorState carries both, but get_state() above only forwarded
        pos/vel/torq so temperature never reached the ROS layer.

        CAVEAT (RobStride): see get_positions() -- the cached MotorState can go
        stale because compact 0x18 report frames are not decoded. Temperature
        may therefore be stale or never populated on RS firmware. Callers MUST
        treat NaN (and a constant 0.0) as "no reading" rather than as a real
        temperature, and must not make safety decisions from missing data.
        """
        if request_feedback:
            self._request_feedback()
        ...
        t_rotor: list[float] = []
        for jc in self._jcfgs:
            st = self._mm[jc.name].get_state()
            if st is None:
                t_mos.append(float("nan"))
                t_rotor.append(float("nan"))
            else:
                t_mos.append(float(getattr(st, "t_mos", float("nan"))))
                t_rotor.append(float(getattr(st, "t_rotor", float("nan"))))
        return (
            np.array(t_mos, dtype=np.float64),
            np.array(t_rotor, dtype=np.float64),
        )
...
class RebotArm:
    ...
    def get_temperatures(
        ...
        MotorState already carries both, but get_state() only forwards
        pos/vel/torq, so temperature was unreachable above this layer.

        request_feedback defaults to False: callers polling at low rate should
        piggyback on the feedback the control loop is already requesting rather
        than injecting extra CAN traffic. A motor that has never reported yields
        NaN so callers can tell "no data" from "genuinely 0 C".
        """
        if request_feedback:
            for m in self._motor_map.values():
                try:
