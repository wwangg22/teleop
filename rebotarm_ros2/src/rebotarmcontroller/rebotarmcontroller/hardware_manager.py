...
# A grasp is only declared when the ramped close target has run at least this
# far ahead of the measured position -- keeps the stall detector from firing
# while the fingers are simply tracking the ramp.
_GRIPPER_STALL_LAG_RAD = 0.1
...
class HardwareManager:
    ...
    def __init__(
        ...
        # RebotArmEndPose calls load_robot_model() with NO argument, so its model
        # comes from the SDK's own config/rebotarm.yaml (hardware_yaml ->
        # urdf_path), NOT from the model:= launch argument. That model drives the
        # 500 Hz loop's gravity feedforward, so if it does not match the arm that
        # is plugged in, tau_ff is wrong -- and between DM and RS it is
        # SIGN-INVERTED on joint2/joint3, the two joints carrying the gravity
        # load. Silent when it happens, and it looks like droop, jitter and hot
        # motors rather than a config error. So: check, and be loud.
        self._verify_kinematics_model(runtime_config)
        ...
        # Grasp-hold tuning (see close_gripper_grasp / configure_gripper).
        # The MIT close command is a pure spring toward the target, so with
        # the target at the mechanical limit any grasped object used to see
        # kp * (whole remaining stroke) -- saturated at the rs-00 MIT frame
        # limit (14 Nm) -- indefinitely. These defaults bound the steady
        # squeeze at ~2 Nm instead.
        self._gripper_hold_torque = 2.0
        self._gripper_close_speed = 3.0
        self._gripper_stall_window = 0.15
        self._gripper_stall_dpos = 0.01
    ...
    def _verify_kinematics_model(self, runtime_config) -> None:
        """Record whether the SDK's kinematics model matches the selected arm.

        This class has no logger, so the result is stored for the ROS node to
        report: `kinematics_model` (the file the SDK actually loaded) and
        `kinematics_warning` (None when it matches).
        """
        self.kinematics_model = "unknown"
        self.kinematics_warning = None
        try:
            from pathlib import Path

            from reBotArm_control_py.kinematics.robot_model import _resolve_urdf

            sdk_urdf, _ = _resolve_urdf(None)
            self.kinematics_model = Path(sdk_urdf).name
            want = str(runtime_config.get("urdf_path", "") or "")
            want_name = Path(want).name if want else ""
            if want_name and self.kinematics_model != want_name:
                self.kinematics_warning = (
                    "KINEMATICS MODEL MISMATCH -- gravity feedforward is being "
                    f"computed on the WRONG ROBOT. The SDK loaded "
                    f"'{self.kinematics_model}' but this driver was launched for "
                    f"'{want_name}'. Between DM and RS the joint2/joint3 axes are "
                    "inverted, so tau_ff comes out sign-inverted on the two joints "
                    "carrying the gravity load: expect droop, jitter and hot "
                    "motors. Fix hardware_yaml in "
                    "third_party/reBotArm_control_py/config/rebotarm.yaml to match "
                    "the arm, then restart the driver."
                )
        except Exception as exc:                      # never block startup on this
            self.kinematics_warning = (
                f"could not verify the SDK kinematics model: {exc}")
    ...
    def cmd_lock_window(self, timeout: float):
        """Bounded-wait command-lock window for periodic readers.

        The motorbridge binding has no locking of its own, so a reader doing
        request/response CAN traffic concurrently with the control loop can
        consume the reply frame the loop is blocking on and stall it for up to
        the SDK's 1 s call timeout. Yields True with the lock held, or False
        (without the lock) if it could not be acquired within `timeout` --
        callers should then SKIP the cycle rather than read unlocked.
        """
        acquired = self._cmd_lock.acquire(timeout=timeout)
        try:
            yield acquired
        finally:
            if acquired:
    ...
    def get_joint_temperatures(
        self,
        request: bool = False,
    ) -> tuple[np.ndarray, np.ndarray]:
        """Arm joint (t_mos, t_rotor) in degrees C; NaN where unavailable."""
        try:
            return self._arm_group.get_temperatures(request_feedback=request)
        except Exception:
            n = len(self.joint_names)
            nan = np.full(n, float("nan"), dtype=np.float64)
            return nan, nan.copy()

    def get_gripper_temperatures(
        self,
        request: bool = False,
    ) -> tuple[float, float]:
        """Gripper (t_mos, t_rotor) in degrees C; NaN where unavailable."""
        if not self.has_gripper:
            return float("nan"), float("nan")
        try:
            t_mos, t_rotor = self._gripper_group.get_temperatures(
                request_feedback=request
            )
        except Exception:
            return float("nan"), float("nan")
        return (
            float(t_mos[0]) if len(t_mos) else float("nan"),
            float(t_rotor[0]) if len(t_rotor) else float("nan"),
        )
    ...
    @_locked
    def set_joint_position_velocity_target(self, positions, velocities) -> None:
        """Position target WITH velocity feedforward.

        set_joint_position_target() forces qd_target to 0, which in MIT mode makes
        the kd term (-kd * qd) brake against the intended motion at every setpoint.
        During a trajectory we know the segment velocity, so feed it forward and let
        the damping term act on velocity ERROR instead of absolute velocity.
        """
        if self._state_machine == "SAFE_HOMING":
            raise RuntimeError("rejecting joint target during safe home")
        target = np.asarray(positions, dtype=np.float64).reshape(-1)
        ...
        if len(vel) != len(self.joint_names):
            raise ValueError(
                f"expected {len(self.joint_names)} joint velocities, got {len(vel)}"
            )
        self.stop_motion()
        self._endpos_ctrl._q_target[:] = target
        self._endpos_ctrl._qd_target[:] = vel
    ...
    def safe_home(self) -> None:
        ...
        try:
            if self.has_gripper:
                # Grasp-aware close: if an object is still held at shutdown,
                # squeeze it at hold_torque rather than the MIT saturation
                # torque until the motors are disabled.
                self.close_gripper_grasp(_GRIPPER_CLOSED_POSITION)
    ...
    def configure_gripper(
        ...
        *,
        ...
        stall_dpos: float | None = None,
    ) -> None:
        """Override the SDK's gripper MIT gains and grasp-hold tuning.

        The packaged hardware yaml ships kp=50/kd=4 for the gripper joint;
        overriding here (instead of forking the SDK config) lets the ROS
        node expose them as parameters. kp/kd take effect on the very next
        MIT send -- including mid-hold, since the 500 Hz loop re-reads the
        group's gain arrays every tick.
        """
        if not self.has_gripper:
            return
        with self._cmd_lock:
            if kp is not None and kp > 0.0:
                self._gripper_group._mit_kp[:] = float(kp)
            if kd is not None and kd >= 0.0:
                self._gripper_group._mit_kd[:] = float(kd)
            if hold_torque is not None and hold_torque > 0.0:
                self._gripper_hold_torque = float(hold_torque)
            if close_speed is not None and close_speed > 0.0:
                self._gripper_close_speed = float(close_speed)
            if stall_window is not None and stall_window > 0.0:
                self._gripper_stall_window = float(stall_window)
            if stall_dpos is not None and stall_dpos > 0.0:
                self._gripper_stall_dpos = float(stall_dpos)
    ...
    def wait_gripper_target(self, timeout: float = 3.0) -> bool:
        ...
        while time.monotonic() < deadline:
            # Bounded lock window: gripper position is a request/response CAN
            # read, and doing it unlocked can eat the 500 Hz loop's RX frame
            # (same failure mode the joint-state publisher had). On a missed
            # window, skip the sample rather than read unlocked.
            with self.cmd_lock_window(0.05) as ok:
                reached = self.gripper_reached_target() if ok else False
            if reached:
                return True
            time.sleep(0.02)
        return False
    ...
    def close_gripper_grasp(
        self,
        position: float = _GRIPPER_CLOSED_POSITION,
        timeout: float = 4.0,
    ) -> tuple[bool, bool, float]:
        """Close toward `position`, treating a stall as a successful grasp.

        Returns (success, grasped, measured_position).

        The close target is RAMPED at _gripper_close_speed instead of being
        stepped to the limit, so an object is met at bounded speed and the
        transient squeeze while the stall detector converges stays near
        kp * close_speed * stall_window instead of the MIT frame's 14 Nm
        saturation. Once the fingers stop moving while the ramp has run
        ahead, the live target is re-anchored to stall_pos +/- delta with
        delta = hold_torque / kp, so the steady squeeze torque is
        ~hold_torque no matter where the object stopped the stroke.
        """
        target = float(position)
        poll_dt = 0.02
        with self.cmd_lock_window(1.0) as ok:
            if not ok:
        ...
        while time.monotonic() < deadline:
            step = self._gripper_close_speed * poll_dt
            if direction > 0:
                ramp = min(ramp + step, target)
            else:
                ramp = max(ramp - step, target)
            with self.cmd_lock_window(0.05) as ok:
                if not ok:
                    time.sleep(poll_dt)
                    continue
                self.set_gripper_target(ramp)
                pos = float(self._gripper_group.get_positions()[0])

            if abs(pos - target) < _GRIPPER_GOAL_TOLERANCE_RAD:
                # Reached the commanded limit: closed on air (or a very thin
                # object). Pin the target to the limit and report reached.
                with self.cmd_lock_window(0.5) as ok:
                    if ok:
            ...
            while history and history[0][0] < now - self._gripper_stall_window:
                history.pop(0)
            span = history[-1][0] - history[0][0]
            moved = abs(history[-1][1] - history[0][1])
            lag = abs(ramp - pos)
            if (
                span >= 0.9 * self._gripper_stall_window
                and moved < self._gripper_stall_dpos
                and lag > _GRIPPER_STALL_LAG_RAD
            ):
                # Object detected: hold with a bounded squeeze instead of
                # kp * (remaining stroke).
                kp = float(self._gripper_group._mit_kp[0])
                delta = self._gripper_hold_torque / max(kp, 1e-6)
                hold_target = pos + direction * delta
                lo, hi = (target, pos) if direction < 0 else (pos, target)
                hold_target = min(max(hold_target, lo), hi)
                with self.cmd_lock_window(0.5) as ok:
                    if ok:
                        self.set_gripper_target(hold_target)
    ...
    # A control cycle sent LATE is far better than a control cycle not sent.
    # In MIT mode each cycle is a fresh impedance setpoint; skip it and the motor
    # coasts on stale torque until the next one lands as a large correction
    # against accumulated error, at kp=150 with no velocity ceiling. Strings of
    # skipped cycles are felt as violent chatter, as if the arm were receiving
    # conflicting commands.
    #
    # And they were being skipped in bulk. Every set_joint_position_target /
    # set_joint_position_velocity_target is @_locked and the trajectory executor
    # plus ChunkExecutor call one at 200 Hz (each nesting a stop_motion(), which
    # takes the lock again). Individually those holds are microseconds, but this
    # process is CPU-bound (~1.7M nonvoluntary context switches on the main
    # thread), so a writer gets preempted WHILE HOLDING the lock and a
    # microsecond hold becomes milliseconds -- taking out several consecutive
    # cycles with a non-blocking acquire.
    #
    # 1.5 ms is inside the 2 ms budget of a 500 Hz loop, so waiting costs at
    # most a fraction of one period. If the holder really is slow (set_zero,
    # stop_motion joining a send thread) the timeout expires and we skip exactly
    # as before -- so this is strictly better than blocking=False, never worse.
    _CMD_LOCK_WAIT = 0.0015

    def _endpos_loop_cb(self, robot, dt: float) -> None:
        del dt
        if not self._cmd_lock.acquire(timeout=self._CMD_LOCK_WAIT):
            return
        try:
