...
class ArmServices:
    ...
    def _move_gripper(self, request, response, default_target: float, label: str):
        try:
            ...
            timeout = request.timeout if request.timeout > 0.0 else 4.0
            if label == "close":
                # Grasp-aware close: stalling on an object is a SUCCESSFUL
                # grasp (held at ~gripper_hold_torque), not a timeout.
                success, grasped, position = self._hardware.close_gripper_grasp(
                    target,
                    timeout=timeout,
                )
                response.message = (
                    f"grasped object at {position:.2f} rad"
                    if grasped
                    else (
                        "gripper close complete"
                        if success
                        else "gripper close timeout"
                    )
                )
            else:
                ...
                response.message = (
                    f"gripper {label} complete"
                    if success
                    else f"gripper {label} timeout"
                )
